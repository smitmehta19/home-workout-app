...
**Navigation behaves the way a phone should.** Detail sheets are part of the history stack, so the back gesture closes the sheet rather than navigating the page underneath it. Sheets can be dragged down to dismiss, and paged with arrows, a sideways swipe or the left/right keys — so you can walk the whole session, or every exercise for a muscle, without closing and reopening.
